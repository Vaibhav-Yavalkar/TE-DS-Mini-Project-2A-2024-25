...
def recommend():
    data = request.json
    job_title = data.get('job_title', '')  # Match key name with JavaScript
    ...
    years_experience = data.get('years_of_experience', '')  # Match key name with JavaScript
    expected_salary = data.get('salary_avg_estimate', '')  # Match key name with JavaScript

    if not job_title and not skills and not qualification and not location:
    ...
    user_input = f"{job_title} {skills} {qualification} {location}"
